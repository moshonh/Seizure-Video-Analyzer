...
""", unsafe_allow_html=True)
        ...
        if not df_features.empty:
            ...
            evol_vals = [safe_float(scores.get("es", 0.0))
                         for scores in (result.feature_scores or {}).values()]

            x      = np.arange(len(names))
            bwidth = 0.38
            fig, ax = plt.subplots(figsize=(10, 4.2))
            ax.bar(x - bwidth / 2, pnes_vals, bwidth,
                   label="PNES-like", color="#16a34a", alpha=0.85)
            ax.bar(x + bwidth / 2, evol_vals,  bwidth,
                   label="Evolving",  color="#2563eb", alpha=0.85)
            ax.set_xticks(x)
            ax.set_xticklabels(names, rotation=30, ha="right", fontsize=8.5)
            ax.set_ylabel("Weighted score")
            ax.set_title("Feature-level score breakdown: PNES-like vs Evolving")
            ax.legend()
