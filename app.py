...
""", unsafe_allow_html=True)
    ...
    # Scale down to canvas width (never upscale)
    ...
    # Passing a data-URI string is the most reliable cross-version way to
    # set the canvas background.  Passing a PIL Image directly is broken in
    # several streamlit-drawable-canvas releases (shows blank white).
    import base64 as _b64mod, io as _iomod
    _buf = _iomod.BytesIO()
    bg_image.save(_buf, format="PNG")
    _b64 = _b64mod.b64encode(_buf.getvalue()).decode("utf-8")
    bg_data_uri = f"data:image/png;base64,{_b64}"

    # ── Static reference image (always visible) ───────────
    st.image(bg_image,
             caption="First frame — draw your ROI on the canvas below",
