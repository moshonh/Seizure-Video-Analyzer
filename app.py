...
def label_for_display(classification: str) -> str:
    return {
        "High-confidence PNES pattern":    "High-confidence PNES pattern",
        "Not confidently PNES":            "Not confidently PNES",
        "Insufficient / poor-quality video": "Insufficient / poor-quality video",
        # Legacy aliases kept for safety
        "PNES":          "High-confidence PNES pattern",
        "Indeterminate": "Not confidently PNES",
        "ES":            "Not confidently PNES",
    }.get(classification, "Not confidently PNES")
...
st.markdown("**Research & Educational Prototype** · High-Specificity PNES Pattern Detector · GDI Framework")
...
This tool is designed to identify <b>typical PNES-like motor patterns with high specificity</b>.
It does <b>not</b> attempt to diagnose all epileptic seizure types or make a general ES-vs-PNES determination.
If the pattern is not clearly and strongly PNES-like, the tool abstains and returns <b>Not confidently PNES</b>.
</div>
""", unsafe_allow_html=True)
...
| Feature | Supports PNES-like pattern | Arguments against PNES |
|---------|---------------------------|------------------------|
| Movement stereotypy | High — repetitive cycles | Low — changing cycles |
| Distribution stability | Stable pattern structure | Drifting / changing distribution |
...
This tool uses a **high-specificity PNES detector** model — it will only output "High-confidence PNES pattern" when multiple PNES-supportive features are jointly present and no strong anti-PNES gates are triggered.

If any of the following are detected, the tool abstains and returns "Not confidently PNES":
- meaningful temporal evolution (rhythm, direction, or amplitude changes)
- progressive variance or amplitude drift
- direction shift from early to late in the recording
- insufficient movement stereotypy
- unstable distribution structure

This conservative approach means the tool may miss some atypical PNES events,
but aims to avoid over-classifying heterogeneous epileptic patterns as PNES.
""")
...
if uploaded_file and analyze_clicked:
    ...
    try:
        ...
        raw_classification = getattr(result, "classification", "Not confidently PNES")
        ...
        pnes_confidence = safe_float(getattr(result, "pnes_pattern_confidence",
                                             getattr(result, "pnes_score", 0.0)))

        # Gate flags for display
        gate_flags    = getattr(result, "gate_flags", {})
        n_triggered   = sum(1 for v in gate_flags.values() if v)
        n_total_gates = len(gate_flags)
...
<div class="{card_class}">
  ...
  <p style="margin:8px 0 0 0">
    ...
    &nbsp;|&nbsp;
    PNES-pattern score: <b>{pnes_confidence:.1f}/10</b>
    &nbsp;|&nbsp;
    Anti-PNES gates triggered: <b>{n_triggered}/{n_total_gates}</b>
  </p>
</div>
""", unsafe_allow_html=True)
        ...
        nm6.metric("Rhythm Irregularity",f"{_f.rhythm_irregularity:.3f}",
                   help=f"CV of inter-peak intervals. High values are anti-PNES (gate threshold: {scoring.PNES_RHYTHM_IRR_MAX}).")
        nm7.metric("Temporal Evolution", f"{_f.temporal_evolution:.3f}",
                   help=f"Composite change in variability, amplitude, and direction. Anti-PNES gate if > {scoring.PNES_EVOLUTION_MAX}.")
        ...
        if not df_features.empty:
            ...
            pnes_vals = df_features["PNES support"].tolist()

            x     = np.arange(len(names))
            fig, ax = plt.subplots(figsize=(9, 3.8))
            colours = ["#16a34a" if v > 0 else "#9ca3af" for v in pnes_vals]
            ax.bar(x, pnes_vals, color=colours, alpha=0.85)
            ax.set_xticks(x)
            ax.set_xticklabels(names, rotation=18, ha="right", fontsize=9)
            ax.set_ylabel("PNES support score")
            ax.set_title("Feature-level PNES pattern support (gate features shown as 0)")
...
st.caption(
    ...
    "High-Specificity PNES Pattern Detector · GDI Framework · "
