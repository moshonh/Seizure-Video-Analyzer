...
""", unsafe_allow_html=True)
    ...
    # ── 3. ROI Selection — sliders + live preview ─────────
    section("3 · Select ROI Around Patient")
    info_box(
        "Use the sliders to position a rectangle around the patient. "
        "The preview updates immediately. "
        "The same ROI will be used for every frame."
    )

    # ── Convert BGR → RGB → PIL for display ──────────────
    frame_rgb = cv2.cvtColor(first_frame, cv2.COLOR_BGR2RGB)
    frame_pil = Image.fromarray(frame_rgb)

    # Default ROI covers the full frame
    default_w = frame_w
    default_h = frame_h

    slider_col, preview_col = st.columns([1, 2], gap="large")

    with slider_col:
        roi_x = st.slider("ROI X (left edge)",   0, max(0, frame_w - 10), 0,          step=1, key="roi_x")
        roi_y = st.slider("ROI Y (top edge)",    0, max(0, frame_h - 10), 0,          step=1, key="roi_y")
        roi_w = st.slider("ROI Width",           10, frame_w,             default_w,  step=1, key="roi_w")
        roi_h = st.slider("ROI Height",          10, frame_h,             default_h,  step=1, key="roi_h")

        # Clamp so ROI never exceeds frame bounds
        roi_x2 = min(roi_x + roi_w, frame_w)
        roi_y2 = min(roi_y + roi_h, frame_h)
        roi_w_clamped = roi_x2 - roi_x
        roi_h_clamped = roi_y2 - roi_y

        st.caption(
            f"x1={roi_x}, y1={roi_y}, x2={roi_x2}, y2={roi_y2} · "
            f"{roi_w_clamped} × {roi_h_clamped} px"
        )

    with preview_col:
        # Draw red rectangle on a copy of the first frame
        preview = first_frame.copy()
        cv2.rectangle(preview, (roi_x, roi_y), (roi_x2, roi_y2), (220, 38, 38), 3)
        preview_rgb = cv2.cvtColor(preview, cv2.COLOR_BGR2RGB)
        preview_pil = Image.fromarray(preview_rgb)

        # Scale for display (max 700 px wide)
        PREVIEW_MAX_W = 700
        disp_scale = min(1.0, PREVIEW_MAX_W / frame_w)
        disp_w = int(frame_w * disp_scale)
        preview_resized = preview_pil.resize((disp_w, int(frame_h * disp_scale)), Image.LANCZOS)
        st.image(preview_resized, caption="ROI preview (red rectangle)", use_container_width=False)

    # Validate ROI
    roi = None
    if roi_w_clamped >= 10 and roi_h_clamped >= 10:
        roi = (roi_x, roi_y, roi_x2, roi_y2)
        st.success(
            f"✅ ROI ready — x1={roi_x}, y1={roi_y}, x2={roi_x2}, y2={roi_y2} "
            f"({roi_w_clamped} × {roi_h_clamped} px in original resolution)"
        )
    else:
        warn_box("ROI is too small. Adjust the sliders so Width and Height are at least 10 px.")
