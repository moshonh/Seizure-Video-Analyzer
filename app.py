...
@st.cache_resource
def _load_rf_model():
    """Load the Random Forest model (cached — only loaded once per session)."""
    import os
    model_path = os.path.join(os.path.dirname(__file__), "random_forest_model.joblib")
    if not _JOBLIB_OK or not os.path.exists(model_path):
        return None
    try:
        import warnings
...
def _build_rf_feature_vector(result, features) -> "pd.DataFrame | None":
    """
    Build the 62-feature DataFrame that the Random Forest expects.
    Mirrors the training pipeline exactly.
    """
    import numpy as _np
    from scipy.signal import find_peaks as _fp

    motion      = _np.asarray(getattr(features, "motion_signal",     []))
    variability = _np.asarray(getattr(features, "variability_signal", []))
    timestamps  = _np.asarray(getattr(features, "timestamps",         []))
    n = min(len(motion), len(variability), len(timestamps))
    if n < 10:
        return None
    ...
    # GDI scores
    d["pnes_score"] = safe_float(getattr(result, "pnes_score", 0))
    d["es_score"]   = safe_float(getattr(result, "es_score",   0))

    # Primary GDI features
    for f in ["temporal_evolution","movement_stereotypy","rhythm_irregularity",
              "angle_variability","amplitude_variability_over_time","vector_change",
              "temporal_distribution_stability","variance_drift"]:
        d[f] = safe_float(getattr(features, f, 0))

    # Motion statistics
    m = motion[:n]
    v = variability[:n]
    d["motion_mean"]     = float(_np.mean(m))
    d["motion_std"]      = float(_np.std(m))
    d["motion_max"]      = float(_np.max(m))
    d["motion_p25"]      = float(_np.percentile(m, 25))
    d["motion_p75"]      = float(_np.percentile(m, 75))
    d["motion_p90"]      = float(_np.percentile(m, 90))
    std_m = float(_np.std(m)) + 1e-9
    d["motion_skewness"] = float(_np.mean(((m - _np.mean(m)) / std_m) ** 3))
    d["motion_kurtosis"] = float(_np.mean(((m - _np.mean(m)) / std_m) ** 4) - 3)

    d["var_mean"] = float(_np.mean(v))
    d["var_std"]  = float(_np.std(v))
    d["var_max"]  = float(_np.max(v))
    d["var_p75"]  = float(_np.percentile(v, 75))
    d["var_p90"]  = float(_np.percentile(v, 90))

    # Baseline / activity
    thr = float(_np.percentile(m, 30))
    d["baseline_threshold"] = thr
    d["active_fraction"]    = float(_np.mean(m > thr))
    d["baseline_fraction"]  = 1.0 - d["active_fraction"]

    # Peak timing
    peak_idx = int(_np.argmax(m))
    d["peak_time_frac"] = peak_idx / max(n - 1, 1)
    d["peak_motion"]    = float(m[peak_idx])
    tail = m[int(n * 0.75):]
    d["post_peak_quiet"] = 1.0 - float(_np.clip(
        (float(_np.mean(tail)) - thr) / (d["peak_motion"] - thr + 1e-6), 0, 1))

    # Slopes
    x = _np.arange(n, dtype=float)
    d["slope_overall"]    = float(_np.polyfit(x, m, 1)[0])
    half = n // 2
    d["slope_first_half"] = float(_np.polyfit(x[:half], m[:half], 1)[0])
    d["slope_second_half"]= float(_np.polyfit(x[half:], m[half:], 1)[0])
    d["slope_ratio"]      = d["slope_second_half"] / (abs(d["slope_first_half"]) + 1e-9)

    # Thirds
    third = n // 3
    d["mean_early"] = float(_np.mean(m[:third]))
    d["mean_mid"]   = float(_np.mean(m[third:2*third]))
    d["mean_late"]  = float(_np.mean(m[2*third:]))
    d["early_late_ratio"]  = d["mean_late"] / (d["mean_early"] + 1e-6)
    overall = float(_np.mean(m)) + 1e-6
    d["escalation_score"]  = float(_np.clip((d["mean_late"] - d["mean_early"]) / overall, -1, 1))

    d["var_early"]      = float(_np.mean(v[:third]))
    d["var_late"]       = float(_np.mean(v[2*third:]))
    var_overall = float(_np.mean(v)) + 1e-6
    d["var_escalation"] = float(_np.clip((d["var_late"] - d["var_early"]) / var_overall, -1, 1))

    # Peaks
    try:
        min_dist = max(2, n // 20)
        peaks, props = _fp(m, distance=min_dist, prominence=float(_np.std(m)) * 0.25)
        d["n_peaks"] = len(peaks)
        if len(peaks) >= 2:
            iv = _np.diff(peaks.astype(float))
            d["peak_interval_mean"] = float(_np.mean(iv))
            d["peak_interval_cv"]   = float(_np.std(iv)) / (float(_np.mean(iv)) + 1e-6)
            d["peak_interval_std"]  = float(_np.std(iv))
        else:
            d["peak_interval_mean"] = d["peak_interval_cv"] = d["peak_interval_std"] = 0.0
        proms = props.get("prominences", _np.array([]))
        if len(proms) > 0:
            d["peak_prominence_mean"] = float(_np.mean(proms))
            d["peak_prominence_cv"]   = float(_np.std(proms)) / (float(_np.mean(proms)) + 1e-6)
        else:
            d["peak_prominence_mean"] = d["peak_prominence_cv"] = 0.0
    except Exception:
        for k in ["n_peaks","peak_interval_mean","peak_interval_cv","peak_interval_std",
                  "peak_prominence_mean","peak_prominence_cv"]:
            d[k] = 0.0

    # Autocorrelation
    try:
        lag = max(2, n // 10)
        mn = m - float(_np.mean(m))
        ac = float(_np.corrcoef(mn[:-lag], mn[lag:])[0, 1])
        d["autocorr_lag10pct"] = ac if not _np.isnan(ac) else 0.0
    except Exception:
        d["autocorr_lag10pct"] = 0.0

    # Derivative
    diffs = _np.abs(_np.diff(m))
    d["motion_derivative_mean"] = float(_np.mean(diffs))
    d["motion_derivative_max"]  = float(_np.max(diffs))
    d["motion_derivative_std"]  = float(_np.std(diffs))

    # Duration
    if len(timestamps) > 1:
        d["duration_s"]    = float(timestamps[-1] - timestamps[0])
        d["fps_effective"] = n / max(d["duration_s"], 1.0)
    else:
        d["duration_s"] = d["fps_effective"] = 0.0

    # One-hot encoded categorical features
    cls = getattr(result, "classification", "")
    d["app_classification_Evolving / not typical PNES"]    = 1 if cls == "Evolving / not typical PNES" else 0
    d["app_classification_Indeterminate / insufficient data"] = 1 if "Indeterminate" in cls else 0
    d["app_classification_Typical PNES pattern"]           = 1 if cls == "Typical PNES pattern" else 0

    eye = getattr(features, "eye_state", "unavailable")
    d["eye_state_closed"] = 1 if eye == "closed" else 0
    d["eye_state_open"]   = 1 if eye == "open"   else 0

    rel = getattr(result, "reliability", "")
    d["reliability_adequate"] = 1 if rel == "adequate" else 0

    conf = getattr(result, "confidence", "")
    d["confidence_high"]     = 1 if conf == "high"     else 0
    d["confidence_low"]      = 1 if conf == "low"      else 0
    d["confidence_moderate"] = 1 if conf == "moderate" else 0

    return pd.DataFrame([d])


def _run_rf_prediction(result, features):
    """
    Run the Random Forest model and return (label, prob_es, prob_pnes) or None.
    """
    model = _load_rf_model()
    if model is None:
        return None
    try:
        X = _build_rf_feature_vector(result, features)
        if X is None:
            return None
        ...
        prob_es   = float(proba[classes.index("Epileptic Seizure (ES)")])   if "Epileptic Seizure (ES)"                  in classes else 0.5
        prob_pnes = float(proba[classes.index("Psychogenic Non-Epileptic Seizure (PNES)")]) if "Psychogenic Non-Epileptic Seizure (PNES)" in classes else 0.5
        return label, prob_es, prob_pnes
    except Exception:
        return None
...
def _extract_timeseries_features(motion: np.ndarray,
                                  variability: np.ndarray,
                                  timestamps: np.ndarray) -> dict:
    """
    Extract rich features from the raw motion and variability time-series.
    These capture what a clinician sees in the graph — shape, timing, dynamics.
    """
    import numpy as _np
    ...
    # ── Basic statistics ──────────────────────────────────────────────────────
    feats["motion_mean"]      = round(float(_np.mean(motion)),    4)
    feats["motion_std"]       = round(float(_np.std(motion)),     4)
    feats["motion_max"]       = round(float(_np.max(motion)),     4)
    feats["motion_p25"]       = round(float(_np.percentile(motion, 25)), 4)
    feats["motion_p75"]       = round(float(_np.percentile(motion, 75)), 4)
    feats["motion_p90"]       = round(float(_np.percentile(motion, 90)), 4)
    feats["motion_skewness"]  = round(float(
        _np.mean(((motion - _np.mean(motion)) / (_np.std(motion) + 1e-9)) ** 3)), 4)
    feats["motion_kurtosis"]  = round(float(
        _np.mean(((motion - _np.mean(motion)) / (_np.std(motion) + 1e-9)) ** 4) - 3), 4)

    feats["var_mean"]         = round(float(_np.mean(variability)), 4)
    feats["var_std"]          = round(float(_np.std(variability)),  4)
    feats["var_max"]          = round(float(_np.max(variability)),  4)
    feats["var_p75"]          = round(float(_np.percentile(variability, 75)), 4)
    feats["var_p90"]          = round(float(_np.percentile(variability, 90)), 4)

    # ── Baseline / activity structure ─────────────────────────────────────────
    threshold = float(_np.percentile(motion, 30))
    feats["baseline_threshold"] = round(threshold, 4)
    active_mask = motion > threshold
    feats["active_fraction"]    = round(float(_np.mean(active_mask)), 4)
    feats["baseline_fraction"]  = round(1.0 - feats["active_fraction"], 4)

    # ── Temporal position of peak ─────────────────────────────────────────────
    peak_idx = int(_np.argmax(motion))
    feats["peak_time_frac"]  = round(peak_idx / max(n - 1, 1), 4)  # 0=start, 1=end
    feats["peak_motion"]     = round(float(motion[peak_idx]), 4)

    # ── Post-peak quiet: does signal return to baseline after peak? ───────────
    tail = motion[int(n * 0.75):]
    tail_mean = float(_np.mean(tail))
    feats["post_peak_quiet"] = round(
        1.0 - float(_np.clip((tail_mean - threshold) /
                              (feats["peak_motion"] - threshold + 1e-6), 0, 1)), 4)

    # ── Slope features ────────────────────────────────────────────────────────
    x = _np.arange(n, dtype=float)
    slope_all = float(_np.polyfit(x, motion, 1)[0])
    feats["slope_overall"]   = round(slope_all, 6)

    # Slope in first vs second half
    half = n // 2
    slope_first = float(_np.polyfit(x[:half], motion[:half], 1)[0])
    slope_second = float(_np.polyfit(x[half:], motion[half:], 1)[0])
    feats["slope_first_half"]  = round(slope_first,  6)
    feats["slope_second_half"] = round(slope_second, 6)
    feats["slope_ratio"]       = round(slope_second / (abs(slope_first) + 1e-9), 4)

    # ── Early vs late amplitude ───────────────────────────────────────────────
    third = n // 3
    mean_early  = float(_np.mean(motion[:third]))
    mean_mid    = float(_np.mean(motion[third:2*third]))
    mean_late   = float(_np.mean(motion[2*third:]))
    overall     = float(_np.mean(motion)) + 1e-6
    feats["mean_early"]        = round(mean_early, 4)
    feats["mean_mid"]          = round(mean_mid,   4)
    feats["mean_late"]         = round(mean_late,  4)
    feats["early_late_ratio"]  = round(mean_late / (mean_early + 1e-6), 4)
    feats["escalation_score"]  = round(float(_np.clip(
        (mean_late - mean_early) / overall, -1, 1)), 4)

    # ── Variability temporal pattern ──────────────────────────────────────────
    var_early = float(_np.mean(variability[:third]))
    var_late  = float(_np.mean(variability[2*third:]))
    feats["var_early"]         = round(var_early, 4)
    feats["var_late"]          = round(var_late,  4)
    feats["var_escalation"]    = round(float(_np.clip(
        (var_late - var_early) / (float(_np.mean(variability)) + 1e-6), -1, 1)), 4)

    # ── Peak detection ────────────────────────────────────────────────────────
    try:
        min_dist = max(2, n // 20)
        peaks, props = _find_peaks(motion, distance=min_dist,
                                   prominence=float(_np.std(motion)) * 0.25)
        feats["n_peaks"] = len(peaks)
        if len(peaks) >= 2:
            intervals = _np.diff(peaks.astype(float))
            feats["peak_interval_mean"] = round(float(_np.mean(intervals)), 4)
            feats["peak_interval_cv"]   = round(
                float(_np.std(intervals)) / (float(_np.mean(intervals)) + 1e-6), 4)
            feats["peak_interval_std"]  = round(float(_np.std(intervals)), 4)
        else:
            feats["peak_interval_mean"] = 0.0
            feats["peak_interval_cv"]   = 0.0
            feats["peak_interval_std"]  = 0.0
        # Prominence stats
        if "prominences" in props and len(props["prominences"]) > 0:
            feats["peak_prominence_mean"] = round(float(_np.mean(props["prominences"])), 4)
            feats["peak_prominence_cv"]   = round(
                float(_np.std(props["prominences"])) /
                (float(_np.mean(props["prominences"])) + 1e-6), 4)
        else:
            feats["peak_prominence_mean"] = 0.0
            feats["peak_prominence_cv"]   = 0.0
    except Exception:
        feats["n_peaks"]              = 0
        feats["peak_interval_mean"]   = 0.0
        feats["peak_interval_cv"]     = 0.0
    ...
    try:
        lag = max(2, n // 10)
        m_norm = motion - float(_np.mean(motion))
        auto = float(_np.corrcoef(m_norm[:-lag], m_norm[lag:])[0, 1])
        feats["autocorr_lag10pct"] = round(auto if not _np.isnan(auto) else 0.0, 4)
    except Exception:
        feats["autocorr_lag10pct"] = 0.0

    # ── Motion derivative (frame-to-frame change rate) ────────────────────────
    diffs = _np.abs(_np.diff(motion))
    feats["motion_derivative_mean"] = round(float(_np.mean(diffs)), 4)
    feats["motion_derivative_max"]  = round(float(_np.max(diffs)),  4)
    feats["motion_derivative_std"]  = round(float(_np.std(diffs)),  4)

    # ── Duration info ─────────────────────────────────────────────────────────
    if len(timestamps) > 1:
        feats["duration_s"] = round(float(timestamps[-1] - timestamps[0]), 2)
        feats["fps_effective"] = round(n / max(feats["duration_s"], 1.0), 2)
    else:
        feats["duration_s"]    = 0.0
        feats["fps_effective"] = 0.0
...
def build_training_row(result, features, video_filename: str,
    ...
    """
    Export one rich CSV row for ML training data collection.

    Columns:
      - Metadata: filename, label, app classification, scores
      - GDI primary features (8)
      - Rich time-series features from the motion graph (~35)
      - Eye state, reliability, confidence
    """
    ...
    # Extract rich time-series features
    motion      = getattr(features, "motion_signal",     np.array([]))
    variability = getattr(features, "variability_signal", np.array([]))
    timestamps  = getattr(features, "timestamps",         np.array([]))

    ts_feats = _extract_timeseries_features(
        np.asarray(motion),
        np.asarray(variability),
        np.asarray(timestamps),
    )

    # Build header + row
    ts_keys = list(ts_feats.keys())
    ...
    row = (
        ...
        + [ts_feats.get(k, "") for k in ts_keys]
...
# Analysis pipeline
# ─────────────────────────────────────────────────────────────────────────────
# Clear cached results if a new video is uploaded
if "last_video_name" not in st.session_state:
    st.session_state.last_video_name = None
if uploaded_file and uploaded_file.name != st.session_state.last_video_name:
    st.session_state.cached_result   = None
    st.session_state.cached_features = None
    st.session_state.cached_html     = None
    st.session_state.cached_json     = None
    st.session_state.cached_warnings = None
...
if uploaded_file and analyze_clicked:
    ...
    try:
        ...
        # Cache results so re-runs (e.g. from selectbox) don't re-analyse
        st.session_state.cached_result   = result
        st.session_state.cached_features = features
        st.session_state.cached_html     = html_report
        st.session_state.cached_json     = json_export
        st.session_state.cached_warnings = pre_warnings
...
""", unsafe_allow_html=True)
        ...
        rf_pred = _run_rf_prediction(result, features)
        if rf_pred is not None:
            rf_label, rf_prob_es, rf_prob_pnes = rf_pred
            rf_is_pnes = "PNES" in rf_label
            rf_color   = "#16a34a" if rf_is_pnes else "#2563eb"
            rf_emoji   = "🟢" if rf_is_pnes else "🔵"
            conf_pct   = int(max(rf_prob_pnes, rf_prob_es) * 100)
            st.markdown(
                f'''<div style="border:2px solid {rf_color}; border-radius:8px;
                    padding:14px 18px; margin-top:12px; background:#f8f9fa;">
                <b>🤖 Random Forest prediction:</b>
                &nbsp; {rf_emoji} <b style="color:{rf_color}">{rf_label}</b>
                &nbsp;|&nbsp; Confidence: <b>{conf_pct}%</b>
                &nbsp;|&nbsp; P(ES): <b>{rf_prob_es:.2f}</b>
                &nbsp;|&nbsp; P(PNES): <b>{rf_prob_pnes:.2f}</b>
                <br><small style="color:#666">⚠ Preliminary model — small training set.
                Use only as supplementary signal alongside GDI rule-based output.</small>
                </div>''',
                unsafe_allow_html=True,
            )
        ...
        with st.expander("📥 Export for ML Training", expanded=False):
            ...
            if clinical_label and clinical_label != "— select —":
                ...
                # Build raw time-series CSV
                _motion_arr = np.asarray(getattr(features, "motion_signal",     []))
                _var_arr    = np.asarray(getattr(features, "variability_signal", []))
                _ts_arr     = np.asarray(getattr(features, "timestamps",         []))
                _n          = min(len(_motion_arr), len(_var_arr), len(_ts_arr))
                if _n > 0:
                    import io as _io2, csv as _csv2
                    _buf2 = _io2.StringIO()
                    _w2   = _csv2.writer(_buf2)
                    _w2.writerow(["video_filename","clinical_label",
                                  "frame_index","time_s",
                                  "motion_magnitude","variability"])
                    for _i in range(_n):
                        _w2.writerow([
                            uploaded_file.name, clinical_label, _i,
                            round(float(_ts_arr[_i]),     4),
                            round(float(_motion_arr[_i]), 6),
                            round(float(_var_arr[_i]),    6),
                        ])
                    _raw_csv = _buf2.getvalue()
                else:
                    _raw_csv = None
                ...
                with dl_a:
                    st.download_button(
                        label=f"⬇️ Features CSV  [{clinical_label}]",
                        data=training_csv.encode("utf-8"),
                        file_name=f"training_{clinical_label}_{stem}.csv",
                        mime="text/csv",
                        use_container_width=True,
                        type="primary",
                        help="8 GDI features + statistics — one row per video",
                    )
                with dl_b:
                    if _raw_csv:
                        st.download_button(
                            label=f"📈 Raw graph CSV  [{clinical_label}]",
                            data=_raw_csv.encode("utf-8"),
                            file_name=f"raw_{clinical_label}_{stem}.csv",
                            mime="text/csv",
                            use_container_width=True,
                            help="Full motion + variability time-series — every graph point",
                        )
                    else:
            ...
            st.caption(
                "Features CSV: one row per video with all computed features. "
                "Raw graph CSV: every data point in the motion graph with timestamps."
            )
...
elif uploaded_file and st.session_state.get("cached_result") is not None:
    # Re-render cached results without re-running analysis
    result      = st.session_state.cached_result
    features    = st.session_state.cached_features
    html_report = st.session_state.cached_html
    json_export = st.session_state.cached_json
    pre_warnings = st.session_state.cached_warnings or []
    metadata    = st.session_state.cached_metadata
    stem        = uploaded_file.name.rsplit(".", 1)[0]

    # Re-display results section (same block as after analysis)
    st.divider()
    st.header("📊 Analysis Results")
...
""", unsafe_allow_html=True)
    ...
    st.caption("All outputs require expert clinical interpretation.")

    # Training export (stable — no re-analysis triggered)
    st.divider()
    st.subheader("🧠 Export for ML Training")
    st.markdown(
        "Assign a **clinical label** and download a CSV row for your training dataset."
    )
    with st.expander("📥 Export for ML Training", expanded=True):
        clinical_label = st.selectbox(
            "Clinical label (your expert judgment)",
            ["— select —", "PNES", "ES", "Indeterminate"],
            help="Your clinical assessment — independent of the app output.",
            key="clinical_label_cached",
        )
        ...
        if clinical_label and clinical_label != "— select —":
            ...
            if _n > 0:
                ...
                _buf3 = _io3.StringIO()
                _w3   = _csv3.writer(_buf3)
                _w3.writerow(["video_filename","clinical_label",
                              "frame_index","time_s",
                              "motion_magnitude","variability"])
                for _i in range(_n):
                    _w3.writerow([
