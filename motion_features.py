...
def _simple_find_peaks(signal: np.ndarray, min_distance: int, prominence: float) -> np.ndarray:
    """Lightweight local-max peak detector to avoid scipy dependency."""
    ...
    for i in range(1, n - 1):
        if x[i] > x[i - 1] and x[i] >= x[i + 1]:
            left_base = max(0, i - min_distance)
            right_base = min(n, i + min_distance + 1)
            local_min = min(float(np.min(x[left_base:i + 1])), float(np.min(x[i:right_base])))
