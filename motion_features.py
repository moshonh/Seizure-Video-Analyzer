...
def _temporal_evolution(motion: np.ndarray,
    ...
    # Use max rather than mean: if ANY axis shows strong evolution the pattern
    # is evolving. This correctly handles cases where amplitude escalates
    # strongly but variability CV or direction change are moderate.
    return float(np.clip(max(var_score, amp_score, dir_score), 0.0, 1.0))
...
def _variance_drift(motion: np.ndarray, variability: np.ndarray, n_windows: int = 6) -> float:
    """
    Detect GRADUAL progressive drift in amplitude and variance over time.

    Key clinical distinction:
      - True progressive drift (Evolving): amplitude increases GRADUALLY and
        CONSISTENTLY across all time windows — small, relatively uniform increments.
      - Single late burst (PNES): most windows are at baseline, then one large
        step change in the last window(s). High correlation with time but
        NON-UNIFORM increments.

    Method:
      - Split into windows; compute mean motion and variability per window.
      - Compute Pearson correlation with window index (captures monotonic trend).
      - Compute smoothness of increments: CV of consecutive differences.
        Gradual drift → uniform diffs → low CV → high smoothness.
        Late burst    → one big diff, rest ~0 → high CV → low smoothness.
      - Final score = raw_correlation_score × smoothness_factor.

    Returns 0–1. Higher = progressive drift consistent with Evolving pattern.
    """
    n = len(motion)
    if n < n_windows * 5:
        return 0.0
    ...
    sa = np.asarray(seg_amp, dtype=float)
    sv = np.asarray(seg_var, dtype=float)
    x  = np.arange(len(sa), dtype=float)
    ...
    amp_corr = max(0.0, _safe_corr(sa))
    var_corr = max(0.0, _safe_corr(sv))

    amp_growth = max(0.0, (sa[-1] - sa[0]) / (abs(sa[0]) + 1e-6))
    var_growth = max(0.0, (sv[-1] - sv[0]) / (abs(sv[0]) + 1e-6))
    amp_growth = float(np.clip(amp_growth / 2.0, 0.0, 1.0))
    var_growth = float(np.clip(var_growth / 2.0, 0.0, 1.0))

    raw = 0.35 * amp_corr + 0.35 * var_corr + 0.15 * amp_growth + 0.15 * var_growth

    def _smoothness(arr: np.ndarray) -> float:
        """
        Returns 1 for perfectly uniform increments (true drift),
        approaching 0 for step-change patterns (isolated late burst).
        """
        d = np.diff(arr)
        if len(d) < 2 or float(np.mean(np.abs(d))) < 1e-9:
            return 0.5
        cv = float(np.std(d)) / (float(np.mean(np.abs(d))) + 1e-9)
        return float(np.clip(1.0 - cv / 2.0, 0.0, 1.0))

    smoothness_factor = (_smoothness(sa) + _smoothness(sv)) / 2.0

    return float(np.clip(raw * smoothness_factor, 0.0, 1.0))
