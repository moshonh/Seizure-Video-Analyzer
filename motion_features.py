...
class MotionFeatures:
    ...
    # How stable the motion/variability distributions remain across time windows.
...
def _normalize_hist(x: np.ndarray, bins: int = 12, value_range=None) -> np.ndarray:
...
def _temporal_distribution_stability(
    ...
    """
    Estimate whether the statistical structure of motion remains stable over time.

    High stability = windows have similar motion/variability distributions → PNES.
    Low stability  = distributions drift over time → ES.
    Returns 0–1.
    """
    n = len(motion)
    if n < n_windows * 5:
    ...
    m_range = (float(np.min(motion)), float(np.max(motion) + 1e-9))
    v_range = (float(np.min(variability)), float(np.max(variability) + 1e-9))
    ...
    for i in range(n_windows):
        ...
        if len(seg_m) < 3 or len(seg_v) < 3:
            continue
        motion_hists.append(_normalize_hist(seg_m, bins=bins, value_range=m_range))
        var_hists.append(_normalize_hist(seg_v, bins=bins, value_range=v_range))
