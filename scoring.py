...
def compute_scores(features: MotionFeatures, video_data: VideoData, weights: Optional[Weights] = None) -> ScoringResult:
    ...
    # --- GDI Evolution & Stagnancy Logic ---
    # Rationale: PNES patterns are stagnant (ratio ~ 1.0), ES patterns evolve (ratio != 1.0)
    evol_ratio = features.gdi_evolution_ratio
    is_stagnant = 0.75 <= evol_ratio <= 1.25
    
    if is_stagnant:
        pnes_score += 5.0  # Strong boost for rhythmic stagnancy
        evol_score -= 2.0
    else:
        evol_score += 4.5  # Boost for biological evolution
        pnes_score -= 2.5
        # If very high evolution/decay, veto PNES call
        if evol_ratio < 0.6 or evol_ratio > 1.8:
            te_blocks_pnes = True
