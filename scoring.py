...
class Weights:
    ...
    temporal_distribution_stability: float = 1.8   # PRIMARY (inverse ES)
...
def _distribution_stability_override(features: MotionFeatures) -> dict:
    """
    Heuristic guardrail:
    irregular or noisy motion that remains statistically stable over time
    should not be pushed toward ES purely because variability is high.
    """
    bonus = {"pnes": 0.0, "es": 0.0, "label": "", "detail": "", "weight": 0.0}
    if (
        features.temporal_distribution_stability >= 0.62
        and features.temporal_evolution <= 0.45
        and features.amplitude_variability_over_time <= 0.40
    ):
        bonus["pnes"] = 0.45
        bonus["label"] = "Stable noisy pattern override → mild PNES"
        bonus["detail"] = "High statistical stability despite noisy motion."
    if (
        features.temporal_distribution_stability <= 0.38
        and (features.temporal_evolution >= 0.55 or features.vector_change >= 0.40)
    ):
        bonus["es"] = 0.35
...
def _generate_explanation(result: "ScoringResult",
    ...
    if cls == "PNES":
        ...
        if features.temporal_distribution_stability > 0.60:
            lines.append(
                "The statistical distribution of motion and local variability remains fairly stable across time windows, "
        ...
        if features.temporal_distribution_stability < 0.40:
            lines.append(
                "The statistical distribution of motion changes across the clip, suggesting that the underlying motor pattern is not remaining in the same state over time."
            )
    else:
        lines.append(
