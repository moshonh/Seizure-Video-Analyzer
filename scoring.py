...
def compute_scores(features: MotionFeatures, video_data: VideoData, weights: Optional[Weights] = None) -> ScoringResult:
    ...
    # --- GDI Evolution/Stagnancy Gate ---
    evol_ratio = features.gdi_evolution_ratio
    is_stagnant = 0.75 <= evol_ratio <= 1.25
    
    if is_stagnant:
        pnes_score += 5.0  # Pathological stagnancy is typical for PNES
        evol_score -= 2.0
    else:
        evol_score += 4.5  # Significant change suggests biological evolution (ES)
        pnes_score -= 2.5
        if evol_ratio < 0.55 or evol_ratio > 1.85:
            te_blocks_pnes = True
