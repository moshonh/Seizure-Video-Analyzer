...
class Weights:
    ...
    movement_stereotypy:         float = 2.0   # PRIMARY (inverse ES)
    amplitude_variability:       float = 1.4   # PRIMARY
    vector_change:               float = 1.3   # PRIMARY
    temporal_distribution_stability: float = 1.2   # PRIMARY but weaker than drift/evolution
    variance_drift:               float = 2.1   # PRIMARY — progressive growth / non-stationarity
...
def _score_temporal_distribution_stability(f: MotionFeatures, weight: float) -> dict:
    """
    HIGH temporal_distribution_stability → same kind of motion statistics over time → PNES.
    LOW  temporal_distribution_stability → drifting distributions over time → ES.
    """
    pnes_raw = f.temporal_distribution_stability
    es_raw   = 1.0 - f.temporal_distribution_stability
    return {
        "pnes":   round(pnes_raw * weight, 3),
        "es":     round(es_raw   * weight, 3),
        "label":  "Stable distributions over time → PNES" if pnes_raw > es_raw else "Changing distributions over time → ES",
        "detail": f"Temporal distribution stability: {f.temporal_distribution_stability:.3f}",
        "weight": weight,
    }


def _score_variance_drift(f: MotionFeatures, weight: float) -> dict:
    """
    HIGH variance_drift → progressive increase in amplitude / local variance over time → ES.
    LOW  variance_drift → no progressive drift → PNES.
    """
    es_raw   = f.variance_drift
    pnes_raw = 1.0 - f.variance_drift
    return {
        "pnes":   round(pnes_raw * weight, 3),
        "es":     round(es_raw   * weight, 3),
        "label":  "Progressive variance/amplitude drift → ES" if es_raw > pnes_raw else "No progressive drift → PNES",
...
def _distribution_stability_override(features: MotionFeatures) -> dict:
    """
    Heuristic guardrails:
    - noisy but statistically stable motion should not be pushed toward ES purely for being irregular
    - progressive growth in amplitude / variance should support ES even if the pattern remains noisy
    """
    bonus = {"pnes": 0.0, "es": 0.0, "label": "", "detail": "", "weight": 0.0}

    if (
        features.variance_drift >= 0.60
        and (features.temporal_evolution >= 0.42 or features.amplitude_variability_over_time >= 0.32)
    ):
        bonus["es"] = 0.55
        bonus["label"] = "Variance drift override → mild ES"
        bonus["detail"] = "Progressive increase in amplitude / local variance over time."
        return bonus

    if (
        features.temporal_distribution_stability >= 0.65
        and features.temporal_evolution <= 0.40
        and features.amplitude_variability_over_time <= 0.32
        and features.variance_drift <= 0.35
    ):
        bonus["pnes"] = 0.35
        bonus["label"] = "Stable noisy pattern override → mild PNES"
        bonus["detail"] = "High statistical stability without progressive drift."
        return bonus

    if (
        features.temporal_distribution_stability <= 0.35
        and (features.temporal_evolution >= 0.55 or features.vector_change >= 0.40 or features.variance_drift >= 0.50)
    ):
        bonus["es"] = 0.25
        bonus["label"] = "Changing distribution override → mild ES"
        bonus["detail"] = "Distribution drift reinforces evolving pattern."
    return bonus
...
def _generate_explanation(result: "ScoringResult",
    ...
    if cls == "PNES":
        ...
        if features.temporal_distribution_stability > 0.60:
            lines.append(
                "The statistical shape of motion and local variability remains fairly stable across time windows, "
                "supporting a stable repetitive pattern rather than progressive transformation."
            )
        if features.variance_drift < 0.35:
            lines.append(
                "There is little evidence of progressive increase in movement amplitude or local variance across the clip."
            )
        ...
        if features.temporal_distribution_stability < 0.40:
            lines.append(
                "The statistical shape of motion changes across the clip, suggesting that the underlying motor pattern is not remaining in the same state over time."
            )
        if features.variance_drift > 0.45:
            lines.append(
                f"Movement amplitude and/or local variance increases progressively across time windows (variance drift: {features.variance_drift:.2f}), "
                "which supports a non-stationary evolving process."
            )
    else:
        lines.append(
...
def compute_scores(
    ...
    feature_scores = {
        ...
        "Distribution Stability": _score_temporal_distribution_stability(features, weights.temporal_distribution_stability),
    ...
    override = _distribution_stability_override(features)
    if override["pnes"] > 0 or override["es"] > 0:
        feature_scores["Distribution Override"] = override
    ...
    if reliability_score < 0.35 or relative_margin < 0.14:
        classification = "Indeterminate"
    elif (
        features.variance_drift >= 0.62
        and features.temporal_evolution >= 0.42
        and es_score >= pnes_score - 0.35
    ):
        classification = "ES"
