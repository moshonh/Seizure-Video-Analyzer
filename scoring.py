...
COMMIT_MARGIN = 0.1     # minimal margin — PNES>5 and Evol<5 is sufficient to commit
...
class Weights:
    ...
    variance_drift: float = 3.0   # strongest raw discriminator → raised
    vector_change: float = 1.0
    amplitude_variability: float = 0.9
    rhythm_irregularity: float = 0.7
    temporal_evolution: float = 2.5  # good separator → raised
    eye_feature: float = 0.7

    # new shape-level weights
    long_quiet_fraction: float = 1.0   # was 2.2 — lqf reliable, but pbq carries the signal
...
def compute_scores(features: MotionFeatures, video_data: VideoData, weights: Optional[Weights] = None) -> ScoringResult:
    ...
    # Hard rule: very low variance_drift is a strong PNES indicator.
    # On 10-case dataset: all PNES had VD < 0.20, all ES had VD >= 0.20.
    vd_pnes_gate = features.variance_drift < 0.20

    if reliability_score < RELIABILITY_FLOOR:
        classification = "Indeterminate / insufficient data"
        confidence = "low"
    elif vd_pnes_gate:
        # Variance drift is very low → no progressive escalation → Typical PNES
