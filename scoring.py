...
class Weights:
    ...
    rhythm_irregularity:   float = 1.0   # high = irregular = ES
    temporal_evolution:    float = 1.0   # high = evolving = ES
    regional_asynchrony:   float = 0.6   # high = asynchronous = mild ES support
...
def _score_rhythm_irregularity(f: MotionFeatures, weight: float) -> dict:
    """
    High rhythm irregularity (irregular inter-peak intervals) → ES.
    Low irregularity (regular rhythm) → PNES.
    rhythm_irregularity is coefficient of variation; typical range 0-3.
    Values > 1.0 indicate clearly irregular rhythm.
    """
    # Normalise: 0 = perfectly regular, 1.0+ = irregular
    norm     = float(min(f.rhythm_irregularity / 1.5, 1.0))
    es_raw   = norm
    pnes_raw = 1.0 - norm
    n_peaks_note = f"rhythm_irregularity (CV): {f.rhythm_irregularity:.3f}"
    return {
        "pnes":   round(pnes_raw * weight, 3),
        "es":     round(es_raw   * weight, 3),
...
def _score_temporal_evolution(f: MotionFeatures, weight: float) -> dict:
    """
    High temporal evolution (variability changes across early/mid/late) → ES.
    Low evolution (stable pattern throughout) → PNES.
    """
    es_raw   = f.temporal_evolution
    pnes_raw = 1.0 - f.temporal_evolution
    return {
        "pnes":   round(pnes_raw * weight, 3),
        "es":     round(es_raw   * weight, 3),
        "label":  "Evolving pattern → ES" if es_raw > pnes_raw else "Stable pattern → PNES",
        "detail": f"Temporal evolution score: {f.temporal_evolution:.3f}",
        "weight": weight,
    }
...
def _score_regional_asynchrony(f: MotionFeatures, weight: float) -> dict:
    """
    High regional asynchrony (upper/middle/lower bands move differently) → mild ES support.
    Low asynchrony (uniform movement) → mild PNES support.
    """
    # Normalise: typical values 0.0-0.5; above 0.3 = notable asynchrony
...
def _generate_explanation(result: "ScoringResult",
    ...
    if cls == "PNES":
        ...
        if features.rhythm_irregularity > 0.8:
            lines.append(
                f"The rhythm of motor activity appears irregular "
                f"(rhythm irregularity index: {features.rhythm_irregularity:.2f}), "
                f"which adds further support for ES."
            )
        if features.temporal_evolution > 0.40:
            lines.append(
                "The pattern of variability evolves noticeably from the beginning to the end of the recording, "
                "consistent with the dynamic nature of an epileptic seizure."
            )
    else:
        lines.append(
